...
Что делает алгоритм:
Определяет, за сколько ходов конь доберётся от точки A до точки B на доске 100×100.

    
Как работает:
Сначала фиксируются все возможные ходы коня (8 направлений).
Алгоритм использует поиск в ширину (BFS) — это как волна, которая расходится от точки A.
Каждая клетка добавляется в очередь с числом ходов, которое потребовалось, чтобы туда попасть.
Клетки помечаются как посещённые, чтобы не заходить туда снова.
Как только доходим до точки B — возвращаем количество сделанных ходов.
    
BFS гарантирует, что найденный путь — минимальный.
